...
1. Fetches the latest version from the external source via `GET /v1/detail/{slug}?source=clawhub`
2. Compares the installed version against the latest
3. Re-runs the full sandboxed install flow (download → hash → scan → install)
4. Updates the pin with the new version and content hash

**Edit detection works the same way.** If you've modified the installed files, `ap update` detects the hash mismatch and prompts before overwriting. If a newer version is available, the prompt asks whether to update anyway. If the version is already current, the prompt offers to restore the original. Use `--force` to bypass the prompt.
