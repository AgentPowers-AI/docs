...
- Uses polling with exponential backoff (up to ~3 minutes) to handle analysis queues
...
If a scan layer cannot run (e.g., the VirusTotal API is temporarily unavailable), it is marked as **skipped** rather than failed. Skipped layers:

- Do not count against the skill's score
- Are shown as `--` in scan results
- Do not prevent listing — only active findings cause a warn or block

This ensures infrastructure issues never block legitimate submissions.
